...
class Trainer:
    ...
    def _eval_loss(self, split="test"):
        ...
        for _ in range(self.eval_steps):
            ...
            losses.append(self._forward_and_loss(x, y).item())
    ...
    def _log(self, step, loss, t0, split="train"):
        elapsed = time.time() - t0
        steps_per_sec = self.log_every / elapsed

        # read LR from scheduler rather than recomputing --
        # confirms the value that was actually applied
        current_lr = self.scheduler.get_last_lr()[0]

        print(
            f"step {step:5d} | "
            f"{split}_loss {loss:.4f} | "
            f"lr {current_lr:.2e} | "
